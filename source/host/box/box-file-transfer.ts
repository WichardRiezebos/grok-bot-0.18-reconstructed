...
export async function uploadFileViaExecDaemon(ctx: Context, accessor: FileTransferAccessor, boxPath: string, data: Uint8Array): Promise<void> { await runBoxShell(ctx, accessor, `mkdir -p -- ${shellSingleQuote(posix.dirname(boxPath))}`); const part = `${boxPath}.sand-${randomBytes(8).toString("hex")}.part`; try { await writeFileBytesViaExecDaemon(ctx, accessor, part, data); await runBoxShell(ctx, accessor, `mv -f -- ${shellSingleQuote(part)} ${shellSingleQuote(boxPath)}`); } catch (error) { await runBoxShell(ctx, accessor, `rm -f -- ${shellSingleQuote(part)}`).catch(() => {}); throw error; } }
