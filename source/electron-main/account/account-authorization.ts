...
export function createDesktopAccountAuthorizer(deps: {
  ...
  return async (slot: string, context: { readonly isStartup: boolean; readonly previousSlot?: string | null }): Promise<boolean> => {
    ...
    const authorized = local ? true : deps.binding == null || deps.descriptorUrl == null ? true : await deps.binding.authorize({ accountSlot: slot, descriptorUrl: deps.descriptorUrl, allowExistingDataClaim: context.isStartup && context.previousSlot === undefined, hasExistingDurableData: deps.hasExistingDurableData });
    if (authorized) {
      const scope = local ? "local" : accountCacheScope(slot);
      ...
      if (local) deps.store.setHasSeenOnboarding?.(true);
    }
    return authorized;
  };
}
