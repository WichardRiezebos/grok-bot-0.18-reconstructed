...
export class SandCursorAuthService {
  ...
  private async runRefreshAccessToken(args: { backendUrl: string; operationEpoch: number; refreshToken: string }): Promise<string> {
    ...
    this.assertCurrentAuthOperation(args.operationEpoch); if (!response.ok) { this.noteRefreshFailure(args.operationEpoch, { kind: "http_status", httpStatus: response.status }); if (response.status === 401 || response.status === 403) { this.advanceAuthOperationEpoch(); this.credentialState = "revoked"; this.reportedLoggedOutStatus = SIGN_IN_CONFIRMATION_FAILED_STATUS; this.profileCache.clear(); this.emitStatus(SIGN_IN_CONFIRMATION_FAILED_STATUS); throw new SandAuthSignInExpiredError(); } throw new Error(`Cursor token refresh failed with HTTP ${response.status}.`); }
