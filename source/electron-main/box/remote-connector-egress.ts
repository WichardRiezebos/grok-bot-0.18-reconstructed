export function createEgressConnectionObserver<TConnection>() {
  ...
  return {
    wrap<T extends { connect(options?: { demand?: boolean }): Promise<TConnection>; recreate?: (...args: any[]) => any; forceRecreate?: (...args: any[]) => any; issueLocalExecDaemonCredential?: (...args: any[]) => any }>(base: T) {
      return {
        connect: async (options?: { demand?: boolean }) => { const dispatched = epoch; try { const connection = await base.connect(options); if (epoch === dispatched) consumer(connection); return connection; } catch (error) { if (epoch === dispatched) consumer(null); throw error; } },
