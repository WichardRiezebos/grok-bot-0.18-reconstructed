...
function RRouterState(){
  const z=H.c(4);
  const[s,e]=de.useState({provider:"openrouter",model:"google/gemini-3.7-flash",computerModel:null,summarizeModel:null,reasoningEffort:"low",computerReasoningEffort:"low",usage:null,local:null,error:null});
  ...
  const h=async r=>{const packs={high:{model:"anthropic/claude-opus-4.6",computerModel:"anthropic/claude-opus-4.6",summarizeModel:"google/gemini-2.5-flash",reasoningEffort:"medium",computerReasoningEffort:"low"},med:{model:"x-ai/grok-4.6",computerModel:"anthropic/claude-haiku-4.5",summarizeModel:"google/gemini-2.5-flash",reasoningEffort:"medium",computerReasoningEffort:"low"},low:{model:"google/gemini-3.7-flash",computerModel:"anthropic/claude-haiku-4.5",summarizeModel:"google/gemini-2.5-flash",reasoningEffort:"low",computerReasoningEffort:"low"}};const pack=packs[r];if(pack==null)return;const i=s;e(o=>({...o,...pack,error:null}));try{const o=await window.desktop.agent.setInferenceRouter(s.provider,pack),l={...o,error:null};e(l);window.dispatchEvent(new CustomEvent("sand-router-provider-changed",{detail:l}))}catch(o){e({...i,error:String(o?.message??o)})}};
...
export async function applyOriginalRendererRouterPatch({ stageRoot }) {
  ...
  if (await isO39Renderer(assetsRoot)) {
    return await applyO39RendererRouterPatch({ stageRoot });
  }
...
// ---------------------------------------------------------------------------
// Grok Bot 0.39.0 renderer patch (web/Dokploy runtime)
//
// Same catalog ("Dune") approach as 0.36 with the 0.39 minified anchors: the
// settings registry array (_0/V_), the static module-graph entry (entrypoint.ts
// map), the lazy view loader (view.tsx map), the settings palette keywords
// (fut), the shared routine-trigger factories (defaults SCt, load zqe, save
// $qe, sentences ECt/mBe), the automation editor chunk (menu, glyph Mn, fields
// wn) and the settings shell chunk (pane import + dialog map I).
// ---------------------------------------------------------------------------

const O39_CHUNK_VIEW_NAME = "chunk-view-router-patch1.js";

const O39_MAIN_REGISTRY_BEFORE = '_0={general:Tx,computer:ile,usage:lle,updates:ole},V_=[{id:"general",label:{id:"Weq9zb"},icon:"settings-gear",entrypoint:_0.general},{id:"computer",label:{id:"9lzViG"},icon:"display",entrypoint:_0.computer}';
const O39_MAIN_REGISTRY_AFTER = '_0={general:Tx,router:globalThis.__RR_ROUTER_E??(globalThis.__RR_ROUTER_E=mc()),computer:ile,usage:lle,updates:ole},V_=[{id:"general",label:{id:"Weq9zb"},icon:"settings-gear",entrypoint:_0.general},{id:"router",label:{id:"Router"},icon:"git-branch",entrypoint:_0.router},{id:"computer",label:{id:"9lzViG"},icon:"display",entrypoint:_0.computer}';

const O39_MAIN_ENTRYPOINT_BEFORE = '"/src/electron-renderer/features/settings/overlay/computer/entrypoint.ts":Jht,';
const O39_MAIN_ENTRYPOINT_AFTER = '"/src/electron-renderer/features/settings/overlay/router/entrypoint.ts":Object.freeze(Object.defineProperty({__proto__:null,default:globalThis.__RR_ROUTER_E},Symbol.toStringTag,{value:"Module"})),' + O39_MAIN_ENTRYPOINT_BEFORE;

const O39_MAIN_VIEW_BEFORE = '"/src/electron-renderer/features/settings/overlay/updates/view.tsx":()=>ot(()=>import("./chunk-view-GC8kiysF.js"),__vite__mapDeps([48,49,40,44]),import.meta.url),';
const O39_MAIN_VIEW_AFTER = O39_MAIN_VIEW_BEFORE + `"${"/src/electron-renderer/features/settings/overlay/router/view.tsx"}":()=>ot(()=>import("./${O39_CHUNK_VIEW_NAME}"),[],import.meta.url),`;

const O39_MAIN_PALETTE_BEFORE = 'const fut={general:["account","model","notifications","preferences","appearance","theme","mode","color","accent","bot","chat","tint","security","yubikey","webauthn"]';
const O39_MAIN_PALETTE_AFTER = 'const fut={router:["router","openrouter","model","models","spend"],general:["account","model","notifications","preferences","appearance","theme","mode","color","accent","bot","chat","tint","security","yubikey","webauthn"]';

const O39_MAIN_TRIG_DEFAULTS_BEFORE = 'function SCt(t){switch(t){case"slack":return{platform:"slack",channel:"",match:"message",keyword:"",emoji:"",bySelf:!1};';
const O39_MAIN_TRIG_DEFAULTS_AFTER = 'function SCt(t){switch(t){case"composio":return{platform:"composio",triggerSlug:"GMAIL_NEW_GMAIL_MESSAGE"};case"slack":return{platform:"slack",channel:"",match:"message",keyword:"",emoji:"",bySelf:!1};';

const O39_MAIN_TRIG_LOAD_BEFORE = 'function zqe(t){switch(t.type){case"cron":return{platform:"schedule",schedule:t.schedule};';
const O39_MAIN_TRIG_LOAD_AFTER = 'function zqe(t){switch(t.type){case"cron":return{platform:"schedule",schedule:t.schedule};case"composio":return{platform:"composio",triggerSlug:typeof t.triggerSlug==="string"?t.triggerSlug:"GMAIL_NEW_GMAIL_MESSAGE"};';

const O39_MAIN_TRIG_SAVE_BEFORE = 'function $qe(t){if(t.platform==="slack"){';
const O39_MAIN_TRIG_SAVE_AFTER = 'function $qe(t){if(t.platform==="composio"){const i=String(t.triggerSlug??"").trim();return i.length===0?null:{type:"composio",triggerSlug:i}}if(t.platform==="slack"){';

const O39_MAIN_TRIG_SENTENCE_BEFORE = 'function ECt(t,e){switch(e.platform){case"slack":return Jqe(t,e);';
const O39_MAIN_TRIG_SENTENCE_AFTER = 'function ECt(t,e){switch(e.platform){case"composio":return e.triggerSlug==="GMAIL_NEW_GMAIL_MESSAGE"?"a new Gmail message arrives":"Composio "+e.triggerSlug+" fires";case"slack":return Jqe(t,e);';

const O39_MAIN_TRIG_EVENT_BEFORE = 'function mBe(t,e){switch(e.type){case"slack":return dBe(t,e);';
const O39_MAIN_TRIG_EVENT_AFTER = 'function mBe(t,e){switch(e.type){case"composio":return e.triggerSlug==="GMAIL_NEW_GMAIL_MESSAGE"?"a new Gmail message arrives":"Composio "+e.triggerSlug+" fires";case"slack":return dBe(t,e);';

const O39_AUTO_MENU_BEFORE = 't.jsx(L.Item,{leading:t.jsx(et,{name:"slack",sizePx:18}),onSelect:()=>Z("slack"),children:t.jsx(T,{id:"bCu1pq"})})';
const O39_AUTO_MENU_AFTER = 't.jsx(L.Item,{leading:t.jsx(Ae,{name:"mail",size:"md"}),onSelect:()=>Z("composio"),children:"New Gmail message"}),' + O39_AUTO_MENU_BEFORE;

const O39_AUTO_GLYPH_BEFORE = 'function Mn(d){switch(d){case"slack":return t.jsx(et,{name:"slack",sizePx:16});';
const O39_AUTO_GLYPH_AFTER = 'function Mn(d){switch(d){case"composio":return t.jsx(Ae,{name:"mail",size:"md"});case"slack":return t.jsx(et,{name:"slack",sizePx:16});';

const O39_AUTO_FIELDS_BEFORE = 'function wn(d){const e=B.c(26),{listener:s,webhookCredential:l,onChange:n,onCommit:a}=d;switch(s.platform){case"slack":';
const O39_AUTO_FIELDS_AFTER = 'function wn(d){const e=B.c(26),{listener:s,webhookCredential:l,onChange:n,onCommit:a}=d;switch(s.platform){case"composio":return t.jsx("span",{children:s.triggerSlug==="GMAIL_NEW_GMAIL_MESSAGE"?"New Gmail message":s.triggerSlug});case"slack":';

const O39_SHELL_IMPORT_BEFORE = 'import{S as b}from"./chunk-general-pane-CTIchh33.js";';
const O39_SHELL_IMPORT_AFTER = `import{RRouterPanel as RRpanel}from"./${O39_CHUNK_VIEW_NAME}";` + O39_SHELL_IMPORT_BEFORE;

const O39_SHELL_IMAP_BEFORE = 'const I={general:b,computer:E,usage:N,updates:U};';
const O39_SHELL_IMAP_AFTER = 'const I={general:b,router:RRpanel,computer:E,usage:N,updates:U};';
...
function RRouterState(){
  ...
  const[s,e]=RRr.useState({provider:"openrouter",model:"google/gemini-3.7-flash",computerModel:null,summarizeModel:null,reasoningEffort:"low",computerReasoningEffort:"low",usage:null,local:null,error:null});
...
const O39_VIEW_IMPORT_BEFORE = 'import{j as RRj,r as RRr,ax as RRfocus,B as RRbt,aF as RRsel,O as RRtxt,w as RRcn}from"./index-Dl1Aho6j.js";\nimport{G as RRsec,a as RRrow}from"./chunk-settings-row-BGwCLXev.js";\nimport{S as RRscroll}from"./chunk-scroll-pane-LA3thSZe.js";';
const O39_VIEW_IMPORT_AFTER = 'import{j as RRj,r as RRr,ax as RRfocus,B as RRbt,aF as RRsel,K as RRtxt,t as RRcn}from"./index-BhgGiPaq.js";\nimport{G as RRsec,a as RRrow}from"./chunk-settings-row-BXbsaODN.js";\nimport{S as RRscroll}from"./chunk-scroll-pane-D3e7pJW9.js";';
const O39_VIEW_INPUTCLASS_BEFORE = 'RRouterInputClass="sand-9f619 sand-1useyqa sand-1717udv sand-c342km sand-1a2a7pz sand-jbqb8w sand-1wd3ewq sand-fifm61 sand-jb2p0i"';
const O39_VIEW_INPUTCLASS_AFTER = 'RRouterInputClass="sand-9f619 sand-h8yej3 sand-10w6t97 sand-exx8yu sand-18d9i69 sand-mkeg23 sand-1ua6jya sand-1wd3ewq sand-jb2p0i sand-fc7y3v"';

const O39_ROUTER_VIEW_CHUNK = O36_ROUTER_VIEW_CHUNK
  .replace(O39_VIEW_IMPORT_BEFORE, O39_VIEW_IMPORT_AFTER)
  .replace(O39_VIEW_INPUTCLASS_BEFORE, O39_VIEW_INPUTCLASS_AFTER);

async function isO39Renderer(assetsRoot) {
  for (const name of await readdir(assetsRoot)) {
    if (!name.endsWith(".js")) continue;
    const source = await readFile(path.join(assetsRoot, name), "utf8");
    if (source.includes(O39_MAIN_REGISTRY_BEFORE)) return true;
  }
  return false;
}

export async function applyO39RendererRouterPatch({ stageRoot }) {
  if (O39_ROUTER_VIEW_CHUNK.includes("index-Dl1Aho6j.js") || O39_ROUTER_VIEW_CHUNK.includes("sand-1useyqa")) {
    throw new Error("0.39 view chunk derivation failed: 0.36 import/class anchors drifted.");
  }
  const assetsRoot = path.join(stageRoot, "dist", "renderer", "assets");
  const viewsRoot = path.join(stageRoot, "dist", "renderer");
  const mainCandidates = [];
  ...
  for (const name of await readdir(assetsRoot)) {
    ...
    const target = path.join(assetsRoot, name);
    const source = await readFile(target, "utf8");
    if (source.includes(O39_MAIN_REGISTRY_BEFORE)) mainCandidates.push({ name, target, source });
    if (source.includes(O39_AUTO_MENU_BEFORE)) autoCandidates.push({ name, target, source });
    if (source.includes(O39_SHELL_IMAP_BEFORE) && source.includes(O39_SHELL_IMPORT_BEFORE)) shellCandidates.push({ name, target, source });
  }
  if (mainCandidates.length !== 1) throw new Error(`Expected one 0.39 settings main chunk, found ${mainCandidates.length}.`);
  if (autoCandidates.length === 0) throw new Error("Expected at least one 0.39 automation detail chunk, found 0.");
  if (shellCandidates.length !== 1) throw new Error(`Expected one 0.39 settings shell chunk, found ${shellCandidates.length}.`);

  const changes = [];
  const mainTransforms = [
    ["settings-entrypoints", O39_MAIN_REGISTRY_BEFORE, O39_MAIN_REGISTRY_AFTER],
    ["catalog-entrypoint", O39_MAIN_ENTRYPOINT_BEFORE, O39_MAIN_ENTRYPOINT_AFTER],
    ["catalog-view", O39_MAIN_VIEW_BEFORE, O39_MAIN_VIEW_AFTER],
    ["palette-keywords", O39_MAIN_PALETTE_BEFORE, O39_MAIN_PALETTE_AFTER],
    ["composio-trigger-defaults", O39_MAIN_TRIG_DEFAULTS_BEFORE, O39_MAIN_TRIG_DEFAULTS_AFTER],
    ["composio-trigger-load", O39_MAIN_TRIG_LOAD_BEFORE, O39_MAIN_TRIG_LOAD_AFTER],
    ["composio-trigger-save", O39_MAIN_TRIG_SAVE_BEFORE, O39_MAIN_TRIG_SAVE_AFTER],
    ["composio-trigger-sentence", O39_MAIN_TRIG_SENTENCE_BEFORE, O39_MAIN_TRIG_SENTENCE_AFTER],
    ["composio-event-sentence", O39_MAIN_TRIG_EVENT_BEFORE, O39_MAIN_TRIG_EVENT_AFTER],
  ];
  changes.push(await applyO39Transforms(mainCandidates[0], mainTransforms));

  const autoTransforms = [
    ["composio-trigger-menu", O39_AUTO_MENU_BEFORE, O39_AUTO_MENU_AFTER],
    ["composio-trigger-glyph", O39_AUTO_GLYPH_BEFORE, O39_AUTO_GLYPH_AFTER],
    ["composio-trigger-fields", O39_AUTO_FIELDS_BEFORE, O39_AUTO_FIELDS_AFTER],
  ];
  for (const candidate of autoCandidates) {
    changes.push(await applyO39Transforms(candidate, autoTransforms));
  }

  const shellTransforms = [
    ["router-view-import", O39_SHELL_IMPORT_BEFORE, O39_SHELL_IMPORT_AFTER],
    ["router-view-dialog-map", O39_SHELL_IMAP_BEFORE, O39_SHELL_IMAP_AFTER],
  ];
  changes.push(await applyO39Transforms(shellCandidates[0], shellTransforms));

  const viewPath = path.join(assetsRoot, O39_CHUNK_VIEW_NAME);
  await writeFile(viewPath, O39_ROUTER_VIEW_CHUNK);
  changes.push({
    role: "router-page",
    path: `dist/renderer/assets/${O39_CHUNK_VIEW_NAME}`,
    created: { bytes: Buffer.byteLength(O39_ROUTER_VIEW_CHUNK), sha256: sha256(O39_ROUTER_VIEW_CHUNK) },
  });

  const record = {
    schemaVersion: 1,
    mode: "original-renderer-039-settings-extension",
    rendererVariant: "0.39.0-web",
    chunks: changes,
    features: ["settings-router-provider", "settings-openrouter-model", "settings-openrouter-computer-model", "settings-openrouter-reasoning-effort", "settings-openrouter-model-search", "settings-local-docker-status", "settings-box-auto-suspend", "settings-composio", "settings-local-profile", "settings-gravatar", "usage-openrouter-cost", "routines-composio-gmail"],
    transformations: [
      "settings-entrypoints", "catalog-entrypoint", "catalog-view", "palette-keywords",
...
async function applyO39Transforms(candidate, transforms) {
  let patched = candidate.source;
  for (const [label, before, after] of transforms) {
    patched = replaceExactlyOnce(patched, before, after, `0.39 ${label}`);
  }
  await writeFile(candidate.target, patched);
  return {
    role: candidate.name,
