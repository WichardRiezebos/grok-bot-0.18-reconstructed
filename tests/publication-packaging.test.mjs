...
test("Router settings use the trusted backend and display recorded inference usage", async () => {
  ...
  const routedPrompt = await readFile(path.join(repoRoot, "source", "host", "runner", "routed-system-prompt.ts"), "utf8");
  ...
  assert.match(preload, /setBoxAutoSuspendIdleMs: \(idleMs: number\) => edge\("setBoxAutoSuspendIdleMs", \{ idleMs \}\)/);
  assert.match(preload, /suspendBox: \(\) => edge\("suspendBox"\)/);
  assert.match(preload, /resumeBox: \(\) => edge\("resumeBox"\)/);
  assert.match(mainRpc, /setBoxAutoSuspendIdleMs: \{ args: "object" \}/);
  assert.match(mainRpc, /suspendBox: \{ args: "none" \}/);
  assert.match(mainRpc, /resumeBox: \{ args: "none" \}/);
  assert.match(rendererPatch, /settings-box-auto-suspend/);
  assert.match(rendererPatch, /Auto-suspend/);
  assert.match(rendererPatch, /Sleep now/);
  assert.match(rendererPatch, /desktop\.agent\.setBoxAutoSuspendIdleMs/);
  assert.match(rendererPatch, /desktop\.agent\.suspendBox\(\)/);
  assert.match(rendererPatch, /desktop\.agent\.resumeBox\(\)/);
  assert.match(mainEdge, /setBoxAutoSuspendIdleMs:/);
  assert.match(mainEdge, /getRegisteredBoxIdleSuspendService/);
  assert.match(mainEdge, /idleMs, suspended:/);
  ...
  assert.match(localDocker, /idle\.guardConnect\(options\?\.demand === true, localConnect\)/);
  assert.match(localDocker, /getRegisteredBoxIdleSuspendService/);
  ...
  assert.match(routedPrompt, /buildSandBaseSystemPrompt\(\{ cloudAgentsEnabled: false \}\)/);
  assert.match(routedPrompt, /Do not announce screenshots, clicks, or waits\. Speak only when you are blocked or done\./);
  assert.match(providers, /maxRetries: 0/);
  assert.match(routedPrompt, /Do not close the box browser/);
  assert.match(routedPrompt, /You do have a screen you can drive/);
  ...
  assert.match(coordinator, /RoutedSlotPromotionError/);
  assert.match(coordinator, /slot-promotion/);
  assert.match(coordinator, /slot: slotName/);
  assert.doesNotMatch(coordinator, /turnNeedsRoutedComputer/);
  ...
  assert.match(routedPrompt, /buildRoutedSystemPrompt\(/);
  assert.match(routedPrompt, /buildSandBaseSystemPrompt\(\{ cloudAgentsEnabled: false \}\)/);
  assert.match(routedPrompt, /No Connect plugins are attached to this turn/);
  ...
  assert.match(routedPrompt, /export function grokRouterSystemPrompt\(/);
  assert.match(coordinator, /getRoutedSystemPromptExtra/);
  assert.match(coordinator, /loadRoutedSystemExtra/);
  assert.match(coordinatorRpc, /getRoutedSystemPromptExtra/);
  assert.match(gatewayProtocol, /getRoutedSystemPromptExtra/);
  assert.match(gatewayProtocol, /reviewRoutedVideoAttachment/);
  assert.match(runnerComposition, /getRoutedSystemPromptExtra/);
  assert.match(runnerComposition, /getExtraSections/);
  assert.match(runnerComposition, /asMemoryPromptStore\(session\.memory\)/);
